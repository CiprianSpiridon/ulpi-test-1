...
- If problems cluster into 3+ independent groups → Proceed to file overlap verification
...
1. `mcp__codemap__search_code("error description")` for each issue — identify files each agent will touch
2. Compare file lists across agents — flag any shared files
3. If overlap exists: merge those issues into one agent, or define clear boundaries
...
Affected files: [file paths identified via mcp__codemap__search_code]
