...
This skill operates fully autonomously. **NEVER** use AskUserQuestion or ask the user any questions during plan enhancement. Make all decisions yourself:
- If scope is ambiguous, use your best judgment
- If agent assignment is uncertain, pick the best match
- If dependencies are unclear, analyze the code and decide
- If the plan needs splitting, split it without asking

Proceed through all 5 phases without interruption. The user will review the final plan via ExitPlanMode.
...
### Identify Files via CodeMap

For each task, use CodeMap to discover affected files — this replaces launching Explore agents:

```
# For each task, run semantic search:
mcp__codemap__search_code("[task description keywords]")

# For known function/type names:
mcp__codemap__search_symbols("[key function/class name]")
```

Populate the "Files (estimated)" column with CodeMap results. This is faster and cheaper than launching Explore agents.
...
- **Files:** [Identified via CodeMap — list actual paths with line ranges]
- **Context:** [Existing patterns found via CodeMap search]
- **Output:** [Expected deliverables]
- **Success criteria:** [How to verify completion]

**Search:** Use `mcp__codemap__search_code` and `mcp__codemap__search_symbols` for codebase navigation. Fall back to Glob/Grep for exact patterns only.
...
### Check 6: CodeMap Used for File Discovery
- [ ] File lists in briefs were populated via CodeMap, not guesswork
- [ ] No Explore agents were launched for tasks CodeMap could answer
- [ ] No A → B → C → A cycles
