...
**Tools to use (priority order):**

1. **CodeMap MCP (ALWAYS FIRST)**
   - `mcp__codemap__search_code("description of what you're looking for")` — semantic search by meaning
   - `mcp__codemap__search_symbols("functionOrClassName")` — find functions, classes, types by name
   - `mcp__codemap__get_file_summary("path/to/file.ts")` — overview before reading full file
2. **Glob** — Find files by exact name pattern (`**/*.test.ts`, `**/auth*.ts`)
3. **Grep** — Find exact strings (imports, error messages, literal patterns)
4. **Read** — Read files identified by CodeMap/Glob/Grep
5. **ast-grep skill** — Structural code pattern searches (AST-based)
6. **Explore agent (LAST RESORT)** — Only when all above fail AND you need multi-round deep investigation

**CodeMap search workflow:**
1. Start with 1-2 CodeMap searches to find relevant files and entry points
2. Read the top results to understand patterns
3. Use Glob/Grep only if CodeMap misses something specific
4. Never launch an Explore agent for something CodeMap already answered

See `references/codemap-search-guide.md` for full search guide with examples.
...
mcp__codemap__search_code("authentication middleware and validation")
mcp__codemap__search_code("API endpoint routing patterns")
mcp__codemap__search_code("user data handling and storage")

# Symbol search — find by name
mcp__codemap__search_symbols("evaluateRules")
mcp__codemap__search_symbols("SessionStore")

# Then read the identified files
Read("packages/guards-engine/src/evaluator.ts")
```
...
- [ ] Used CodeMap MCP for initial codebase search (not Explore agents)
...
- **CodeMap MCP** — Always use CodeMap for codebase search in the Gather Context step. See `references/codemap-search-guide.md`.
- **Explore agent** - LAST RESORT during context gathering, only when CodeMap + Glob/Grep are insufficient
