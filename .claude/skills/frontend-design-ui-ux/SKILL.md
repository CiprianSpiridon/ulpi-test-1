...
**Context Analysis:**
- Existing patterns in the codebase — use `mcp__codemap__search_code("UI pattern for [feature]")` to discover existing components, layouts, and design patterns before designing new ones
- Design system constraints
- Tech stack limitations
- Performance requirements
- Accessibility requirements (WCAG 2.1 AA minimum)

**Codebase Pattern Discovery (CodeMap):**

Before designing, search for existing UI patterns:
1. `mcp__codemap__search_code("component similar to [feature]")` — find existing components to reuse or extend
2. `mcp__codemap__search_symbols("Component")` — find all existing component names
3. `mcp__codemap__get_file_summary("path/to/component.tsx")` — understand existing component structure
