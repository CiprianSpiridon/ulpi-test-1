...
If all conditions for parallelization are met, proceed to file overlap verification.

**File Overlap Verification (CodeMap):**

Before proceeding, use CodeMap to verify agents won't collide:

1. `mcp__codemap__search_code("feature description")` for each task — identify files each agent will touch
2. Compare file lists across agents — flag any shared files
3. If overlap exists: merge those tasks into one agent, or define clear boundaries (e.g., "only modify lines 50-80")
...
- Files: [relevant paths identified via mcp__codemap__search_code]
- Requirements: [bullet points]
- Output: [expected deliverables]
- Patterns: [existing code patterns to follow]
- Search: Use mcp__codemap__search_code for semantic search, mcp__codemap__search_symbols for functions/types. Fall back to Glob/Grep for exact patterns.
```
